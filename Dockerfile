...
SHELL ["conda", "run", "--no-capture-output", "-n", "kilroy-face-debug", "/bin/bash", "-c"]

WORKDIR /app/kilroy_face_debug/

# add poetry files
COPY ./kilroy_face_debug/pyproject.toml ./kilroy_face_debug/poetry.lock ./
...
COPY ./kilroy_face_debug/src/ ./src/
COPY ./kilroy_face_debug/tests/ ./tests/
COPY ./kilroy_face_debug/LICENSE ./kilroy_face_debug/README.md ./

# build wheel by poetry and install by pip (to force non-editable mode)
RUN poetry build -f wheel && \
    python -m pip install --no-deps --no-index --no-cache-dir --find-links=dist kilroy-face-debug

RUN --mount=type=cache,target=/etc/torch-tmp \
    TORCH_HOME=/etc/torch-tmp kilroy-face-debug-fetch-models && \
    mkdir -p /etc/torch && \
...
ENTRYPOINT ["./entrypoint.sh", "kilroy-face-debug"]
CMD []
